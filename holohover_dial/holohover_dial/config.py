...
class PathPlannerConfig:
    ...
    def from_ros_node(cls, node: Node) -> 'PathPlannerConfig':
        ...
        """
        return cls(
            planning_horizon=node.declare_parameter('path_planner.planning_horizon', 2.0),
            goal_x=node.declare_parameter('path_planner.goal_x', 0.0),
            goal_y=node.declare_parameter('path_planner.goal_y', 0.0),
            home_x=node.declare_parameter('path_planner.home_x', 0.0),
            home_y=node.declare_parameter('path_planner.home_y', 0.0),
            max_velocity=node.declare_parameter('path_planner.max_velocity', 2.0),
            update_rate=node.declare_parameter('path_planner.update_rate', 30.0),
        )
...
class ControlConfig:
    ...
    def from_ros_node(cls, node: Node) -> 'ControlConfig':
        ...
        """
        return cls(
            position_weight=node.declare_parameter('control.position_weight', 1.0),
            velocity_weight=node.declare_parameter('control.velocity_weight', 0.5),
            control_limit=node.declare_parameter('control.control_limit', 1.0),
            update_rate=node.declare_parameter('control.update_rate', 50.0),
            kp=node.declare_parameter('control.kp', 1.0),
            kd=node.declare_parameter('control.kd', 0.5),
        )
