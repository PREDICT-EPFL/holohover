 ...
 * Copyright (C) 2024 Roland Schwan (message size extensions)
 */
 ...
 * (or mdev with busybox) to create and destroy the /dev/spideveB.C device
		...
		unsigned *n_ioc)
{
	...
	if (_IOC_TYPE(cmd) != SPI_IOC_MAGIC
			|| (_IOC_NR(cmd) != _IOC_NR(SPI_IOC_MESSAGE(0))
                && _IOC_NR(cmd) != _IOC_NR(SPI_IOC_MESSAGE_E(0)))
 ...
 * /dev/spideveB.C character device nodes exposing our userspace API.
...
{
	if (device_property_match_string(dev, "compatible", "spideve") < 0)
		return 0;

	dev_err(dev, "spideve listed directly in DT is not supported\n");
	return -EINVAL;
}
...
MODULE_DESCRIPTION("Extended user mode SPI device interface");
MODULE_LICENSE("GPL");
MODULE_ALIAS("spi:spideve");
