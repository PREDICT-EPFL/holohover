...
*/
#ifndef HOLOHOVER_DRIVERS_MSP_HPP
#define HOLOHOVER_DRIVERS_MSP_HPP
...
public:
    ...
    bool begin(const char *device, unsigned int baud);
    void stop();
    ssize_t send(uint8_t messageID, uint8_t* payload, uint8_t size);
    ssize_t recv(uint8_t* message_id, uint8_t* payload, uint8_t payload_size, int timeout);
    ssize_t wait_for(uint8_t message_id, uint8_t* payload, uint8_t payload_size, int timeout);
    ssize_t request(uint8_t message_id, uint8_t* payload, uint8_t payload_size, int timeout);
    ssize_t command(uint8_t message_id, uint8_t* payload, uint8_t payload_size, int timeout);
...
#endif //HOLOHOVER_DRIVERS_MSP_HPP
